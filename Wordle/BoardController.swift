...
if a != nil {
  ...
  private func applyThemeSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      let x = settings[kWordThemeKey]
if x != nil {
    if let Jstring = x as? String {
if let theme = WordTheme(rawValue: Jstring) {
  ...
  private func applyIsAlienWordleSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
          let name1 = settings[kIsAlienWordleKey]
    if name1 != nil {
if let bool1 = name1 as? Bool {
        isAlienWordle = bool1
    }}
    // ...
    // START YOUR CODE HERE
